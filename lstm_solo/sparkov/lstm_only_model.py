"""
lstm_only_model.py — solo LSTM baseline for the Sparkov dataset (no graph
component at all).

Sibling of gatv2_model.py / lstm_gat_sequential_model.py / lstm_gat_parallel_model.py.
See ibm/lstm_only_model.py for the full rationale: this is the missing
ablation corner that isolates the LSTM's standalone contribution, with
zero relational signal, for direct comparison against the GATv2-only
baseline and the two hybrids.

Because this model has no edge_index, graph_strategy is not a variable
here -- the model trains and evaluates exactly ONCE per dataset, not
once per strategy (see ibm/lstm_only_model.py for why).

Reuses unchanged from utils.py: FoldPreprocessor, split_groups_holdout,
build_stratified_folds, prepare_tensors, choose_threshold, eval_from_probs,
print_metrics, save_results_plots, save_strategy_comparison,
save_results_csv, print_final_comparison. No AMP is used, matching
Sparkov's plain-precision utils.train_loop convention (unlike IBM's
AMP-enabled loop).
...
from utils import (
    ...
    prepare_tensors, _make_sort_key,
...
def _build_seq_indices(df_raw, cfg):
    """Chunks oversized groups into windows instead of truncating --
    see ibm/lstm_gat_sequential_model.py for the full rationale."""
...
class LSTMOnlyFraudModel(nn.Module):
    ...
    def forward(self, x_cat_dict, x_num, groups):
...
# ============================================================================
#  TRAIN LOOP  (no AMP, no edge_index -- matches Sparkov's plain-precision convention)
# ============================================================================
def train_loop_lstm(model, cat_tr_d, num_tr_t, y_tr_t, groups_tr,
                    cat_va_d, num_va_t, y_va_np, groups_va,
                    cfg, verbose=False):
    ...
    def val_ap():
        model.eval()
        probs = torch.sigmoid(model(cat_va_d, num_va_t, groups_va)).cpu().numpy()
        return average_precision_score(y_va_np, probs) if len(np.unique(y_va_np)) == 2 else 0.0

    for ep in range(1, cfg.MAX_EPOCHS + 1):
        model.train(); opt.zero_grad()
        loss = crit(model(cat_tr_d, num_tr_t, groups_tr), y_tr_t)
...
def train_one_fold(df_tr, df_va, cfg, fold_idx=0, device=None):
    ...
    model = train_loop_lstm(model, cat_tr_d, num_tr_t, y_tr_t, groups_tr,
                            cat_va_d, num_va_t, y_va_np, groups_va, cfg, verbose)
    model.eval()
    with torch.no_grad():
        p_tr = torch.sigmoid(model(cat_tr_d, num_tr_t, groups_tr)).cpu().numpy()
        p_va = torch.sigmoid(model(cat_va_d, num_va_t, groups_va)).cpu().numpy()
...
def run_test_ensemble_lstm(all_out, df_test, cfg, device):
    ...
    for fold_i, out in enumerate(all_out):
        ...
        out["model"].eval()
        with torch.no_grad():
            probs = torch.sigmoid(out["model"](cat_te_d, num_te_t, groups_te)).cpu().numpy()
