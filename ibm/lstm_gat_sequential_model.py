...
def _build_seq_indices(df_raw, cfg):
    ...
    Returns a list of (start_offset, [sorted_node_indices]) is NOT what we
    want for variable-length batching; instead we return, per node, its
    position within its group's chronological sequence plus the full
    sorted index list for that group, packaged as a list of index arrays
    (one array per group) -- this is what the LSTM branch consumes.
    ...
    for _, idx in df_raw.groupby(cfg.INTRA_GROUP_KEY).groups.items():
        idx = list(idx)
        if len(idx) > cfg.INTRA_MAX_GROUP_SIZE:
            idx = idx[:cfg.INTRA_MAX_GROUP_SIZE]  # keep deterministic (no shuffle) for LSTM order
        if len(idx) == 0:
            continue
        idx_sorted = sorted(idx, key=lambda i: sort_key[i])
        groups.append(idx_sorted)
    return groups


def _run_lstm_over_groups(lstm, x, groups, device):
    """
    x: [N, F] node features (output of encode()).
    groups: list of variable-length index lists (one per card), from
            _build_seq_indices.
    ...
    # Scatter each timestep's output back to its own node (causal, not broadcast)
    for i, g in enumerate(groups):
...
class LSTMGATFraudModel(nn.Module):
    ...
    def __init__(self, dense_in_dim, high_card_cols, low_card_cols,
        ...
        # LSTM stage. Default lstm_hidden = hidden*heads so GAT's input dim
        # after LSTM matches its usual in_dim scale -- keeps this a fair
        # capacity comparison against GATFraudModel rather than a bigger model.
        ...
        # GAT stage operates on LSTM output, exactly like gat1/gat2 in
        # GATFraudModel operated on raw encode() output.
    ...
    def forward(self, x_dense, x_high, x_low, edge_index, groups):
        ...
        x = _run_lstm_over_groups(self.lstm, x, groups, x.device)
...
def train_one_fold(df_tr, df_va, cfg, graph_strategy, fold_idx=0, device=None):
    ...
    # The one new step: sequence grouping, independent of graph_strategy.
