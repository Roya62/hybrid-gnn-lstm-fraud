...
feeding into GAT as a pipeline stage.
  ...
  encode() -> LSTM branch  -> lstm_features  \
                                                -> cross-attention fusion -> cls
  encode() -> GAT branch   -> gat_features   /
...
def _build_seq_indices(df_raw, cfg):
    ...
    for _, idx in df_raw.groupby(cfg.INTRA_GROUP_KEY).groups.items():
        ...
        if len(idx) > cfg.INTRA_MAX_GROUP_SIZE:
            idx = idx[:cfg.INTRA_MAX_GROUP_SIZE]
        if len(idx) == 0:
            continue
        groups.append(sorted(idx, key=lambda i: sort_key[i]))
    return groups
...
class LSTMGATParallelFraudModel(nn.Module):
    ...
    def __init__(self, dense_in_dim, high_card_cols, low_card_cols,
        ...
        # LSTM branch reads the SAME in_dim input as the GAT branch (parallel,
        # not sequential -- neither branch's output feeds the other).
        ...
        # GAT branch, identical hidden/heads to GATFraudModel for a fair
        # capacity comparison.
    ...
    def forward(self, x_dense, x_high, x_low, edge_index, groups):
        ...
        lstm_feat = _run_lstm_over_groups(self.lstm, x, groups, x.device)
