...
Sibling of gatv2_model.py, following the same conventions:
  - forward(cat_dict, num_tensor, edge_index[, groups]) — Sparkov uses one
    unified categorical embedding dict (EMBEDDING_DIM=8 for every column),
    not the high/low-cardinality split used by the IBM pipeline.
  - FoldPreprocessor(cat_cols, num_cols) + prepare_tensors() from utils.py,
    reused unchanged.
  - No AMP/GradScaler here, matching utils.train_loop's plain-precision
    training (Sparkov's utils.py doesn't use autocast at all).

LSTM sequences are grouped by cfg.INTRA_GROUP_KEY ("cc_num") and ordered by
the same cfg.SORT_KEY_COLS ({"timestamp": 1}) used by _build_intra_group_edges,
so sequence construction is independent of which graph_strategy is active.
Each node receives the LSTM's output at ITS OWN position in its card's
chronological sequence (causal), not the sequence's final hidden state
broadcast to every transaction — broadcasting would leak later transactions'
influence into earlier ones within the same card.
...
def _build_seq_indices(df_raw, cfg):
    """Group by cfg.INTRA_GROUP_KEY ('cc_num'), sort by cfg.SORT_KEY_COLS
    ('timestamp') — the same grouping/ordering _build_intra_group_edges
    already uses, so this is independent of graph_strategy.

    Oversized groups are CHUNKED into windows of INTRA_MAX_GROUP_SIZE, not
    truncated -- truncating silently left every transaction past the cap
    as an unwritten (all-zero) LSTM output fed straight into GAT. See the
    IBM version of this function for the full explanation."""
...
#  MODEL
# ============================================================================
class LSTMGATSeqFraudModel(nn.Module):
    """LSTM encodes each card's chronological sequence; GAT then reasons
    over the LSTM output (not the original cat/num features)."""
...
# ============================================================================
#  TRAIN LOOP  (mirrors utils.train_loop, extended with groups; no AMP,
#  matching Sparkov's utils.py which trains in plain precision)
# ============================================================================
...
# ============================================================================
#  TEST ENSEMBLE  (utils.run_test_ensemble doesn't know about `groups`,
#  so this is a thin local copy rather than a modification of utils.py)
# ============================================================================
...
def run_pipeline(df, cfg, graph_strategy="multi_relation"):
    ...
    print(f"\n{'#'*60}\n# LSTM→GAT (sequential) | {graph_strategy}\n{'#'*60}")
